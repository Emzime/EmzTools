...
local function LoadConfig()
    ...
    if EmzTools.Data and EmzTools.Data.Database then
        ...
        config.showTime = EmzTools.Data.Database:Get("InfoBar", "showTime", true)
...
local function SaveConfig()
    if EmzTools.Data and EmzTools.Data.Database then
        ...
        EmzTools.Data.Database:Set("InfoBar", "showTime", config.showTime)
...
-- Fonction pour synchroniser les boutons radio d'un groupe
local function SyncRadioGroup(groupName, selectedButton)
    if not radioButtonGroups[groupName] then return end

    for _, button in pairs(radioButtonGroups[groupName]) do
...
function InfoBarTab:CreateContent(parentFrame, yOffset)
    ...
    -- 20px radio
    controls.height24Radio, controls.height24Text = CreateRadioButton(
        parentFrame,
        "20px",
        100,
        currentY,
    ...
    ApplyTextColor(controls.height24Text, textColor)
        ...
        if not StaticPopupDialogs["EMZTOOLS_RESET_INFOBAR"] then
            StaticPopupDialogs["EMZTOOLS_RESET_INFOBAR"] = {
                ...
                OnAccept = function()
                    ...
                    -- Hauteur
                    if controls.height24Radio then
                        controls.height24Radio:SetChecked(1)
                    end
...
function InfoBarTab:UpdateControls()
    ...
    -- Hauteur
    if controls.height24Radio then
        controls.height24Radio:SetChecked(config.height == 24 and 1 or 0)
    end
    if controls.height16Radio then
