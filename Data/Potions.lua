...
EmzTools.Data.Potions = EmzTools.Potions
...
-- ===================================================
-- CONFIGURATION SIMPLIFIÉE (sans seuils)
-- ===================================================

-- Configuration par défaut pour les potions
local defaultConfig = {
    autoHeal = true,
    autoMana = true,
    healPriority = "smart", -- "healthstone", "potion", "smart"
    soundAlert = false,
    announce = false,
    useHealthstone = true,
    offensivePriority = false
}
...
function EmzTools.Data.Potions:GetBestHealthPotion()
    ...
    for i = table.getn(self.HealthPotions), 1, -1 do
...
function EmzTools.Data.Potions:GetBestManaPotion()
    ...
    -- Chercher de la plus puissante à la plus faible
    for i = table.getn(self.ManaPotions), 1, -1 do
...
function EmzTools.Data.Potions:GetBestHealthstone()
    ...
    -- Chercher de la plus puissante à la plus faible
    for i = table.getn(self.Healthstones), 1, -1 do
...
function EmzTools.Data.Potions:UseHealthPotion()
    ...
    if EmzTools.Data.Utils and EmzTools.Data.Utils.UseItemByName then
        if EmzTools.Data.Utils:UseItemByName(potion.name) then
            EmzTools:Print("EMZTOOLS_POTIONS_HEALTH_USED", "success", potion.name)
            return true
        end
    end
...
function EmzTools.Data.Potions:UseManaPotion()
    ...
    if EmzTools.Data.Utils and EmzTools.Data.Utils.UseItemByName then
        if EmzTools.Data.Utils:UseItemByName(potion.name) then
            EmzTools:Print("EMZTOOLS_POTIONS_MANA_USED", "success", potion.name)
            return true
        end
    end
...
function EmzTools.Data.Potions:UseHealthstone()
    ...
    if EmzTools.Data.Utils and EmzTools.Data.Utils.UseItemByName then
        if EmzTools.Data.Utils:UseItemByName(stone.name) then
            EmzTools:Print("EMZTOOLS_POTIONS_HEALTHSTONE_USED", "success", stone.name)
            return true
        end
    end
...
function EmzTools.Data.Potions:SmartHeal()
    ...
    if hpPercent >= 90 then
        EmzTools:Print("EMZTOOLS_POTIONS_HEALTH_FULL", "info")
        return false
    end

    -- Vérifier si le joueur est un démoniste
    local isWarlock = false
    if EmzTools.Data.Utils and EmzTools.Data.Utils.GetPlayerClass then
        isWarlock = (EmzTools.Data.Utils:GetPlayerClass() == "WARLOCK")
    end

    -- Récupérer la configuration de priorité depuis la base de données
    local healPriority = "smart"
    if EmzTools.Data and EmzTools.Data.Database then
        local potionConfig = EmzTools.Data.Database:Get("Potions", "config", {})
        healPriority = potionConfig.healPriority or "smart"
    end

    -- Logique de priorité selon la configuration
    if healPriority == "stone_first" or (isWarlock and healPriority == "smart") then
        -- Priorité pierre de soin (spécialement pour les démonistes)
        if self:GetBestHealthstone() then
            return self:UseHealthstone()
        elseif self:GetBestHealthPotion() then
            return self:UseHealthPotion()
        end
    elseif healPriority == "potion_first" then
        -- Priorité potions
        if self:GetBestHealthPotion() then
            return self:UseHealthPotion()
        elseif self:GetBestHealthstone() then
            return self:UseHealthstone()
        end
    else
        -- Mode intelligent par défaut : pierre de soin d'abord pour démonistes
        if isWarlock and self:GetBestHealthstone() then
            return self:UseHealthstone()
        elseif self:GetBestHealthPotion() then
            return self:UseHealthPotion()
        elseif self:GetBestHealthstone() then
...
function EmzTools.Data.Potions:PrintAvailablePotions()
    EmzTools:Print("EMZTOOLS_POTIONS_AVAILABLE_TITLE", "info")
...
function EmzTools.Data.Potions:PrintRecommendedPotions()
    ...
    EmzTools:Print("EMZTOOLS_POTIONS_RECOMMENDED_TITLE", "info", level)
...
local function EnsurePotionsCompatibility()
    if EmzTools.Potions and EmzTools.Data.Potions then
        -- Copier toutes les fonctions si nécessaire
        for funcName, func in pairs(EmzTools.Potions) do
            if type(func) == "function" then
                EmzTools.Data.Potions[funcName] = func
            end
        end
    end
end
